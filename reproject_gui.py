...
def main():
    ...
    while 1:
        ...
        point_and_image_save = 255*np.zeros((512, 640, 3), np.uint8)
        ...
        for i, img_point in enumerate(img_points.xyz):
            ...
            cv2.circle(point_and_image_save, (int(img_point[0]), int(img_point[1])), 2, color, -1, 4, 0)
        ...
        point_and_image_save =cv2.resize(point_and_image_save, (width, height))
        change_flag = 0
        #cv2.imwrite(data_folder+save_reproject+"image"+str(j)+".jpg", point_and_image)
        cv2.imshow(title_window, point_and_image)
        if cv2.waitKey(33) == ord('s'):
            save_img_path = image_path.split(".")[0] + "_reproject" + ".jpg"
            cv2.imwrite(save_img_path, point_and_image_save)
